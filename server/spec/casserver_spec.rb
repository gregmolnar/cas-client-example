...
describe 'CASServer' do
  ...
  describe "/login" do
    ...
    it "logs in successfully with valid username and password and prevents redirection to a malicious service" do
      visit "/login?service="+CGI.escape('http://badguy.com#my.app.test')

      fill_in 'username', :with => VALID_USERNAME
      fill_in 'password', :with => VALID_PASSWORD

      click_button 'login-submit'
      page.current_url.should_not =~ /^#{Regexp.escape('http://badguy.com#my.app.test')}\/?\?ticket=ST\-[1-9rA-Z]+/
    end
